...
parser = argparse.ArgumentParser(
    description="Script for processing robot demonstrations"
)
...
cmd_states_file_name = "commanded_states"
...
for TASK_NAME in task_names:
    ...
    for num, demo_dir in enumerate(sorted(DEMO_DIRS)):
        ...
        if not Path(f"{demo_dir}/states.pkl").exists():
            continue
        states = pkl.load(open(f"{demo_dir}/states.pkl", "rb"))
        cmd_states = pkl.load(open(f"{demo_dir}/commanded_states.pkl", "rb"))

        state_positions = [state.pos for state in states]
        cmd_state_positions = [state.pos for state in cmd_states]
        ...
        cmd_state_orientations = [
            R.from_quat(state.quat).as_rotvec() for state in cmd_states
        ]
        cmd_gripper_positions = [state.gripper for state in cmd_states]
        cmd_state_timestamps = [state.timestamp for state in cmd_states]
        ...
        cmd_state_positions = np.array(cmd_state_positions)
        cmd_state_orientations = np.array(cmd_state_orientations)
        cmd_state_positions = np.concatenate(
            [cmd_state_positions, cmd_state_orientations], axis=1
        )
        cmd_gripper_positions = np.array(cmd_gripper_positions)
        cmd_gripper_positions = cmd_gripper_positions.reshape(-1, 1)
        state_timestamps = np.array(state_timestamps)
        cmd_state_timestamps = np.array(cmd_state_timestamps)
        ...
        for i in range(len(state_positions) - 1):
            if state_start_teleop[i] == False and static == False:
                ...
                state_start_teleop[i] == True
                and (
                    first_step_done
                    or not np.allclose(
                        state_positions[0], state_positions[i + 1], atol=1e-2
                    )
                )
                and static == True
            ):
        ...
        cmd_state_positions_test = cmd_state_positions
        for i in range(len(cmd_state_positions_test)):
            cmd_state_positions_test[i] = np.array(cmd_state_positions_test[i])
        cmd_state_positions_test = pd.DataFrame(
            {"column": [list(row) for row in cmd_state_positions_test]}
        )

        # convert left_gripper to True and False
        gripper_positions_test = pd.DataFrame(gripper_positions)
        cmd_gripper_positions_test = pd.DataFrame(cmd_gripper_positions)

        state_test = pd.concat(
            [
                state_timestamps_test,
                state_positions_test,
                gripper_positions_test,
                cmd_state_positions_test,
                cmd_gripper_positions_test,
            ],
            axis=1,
        )
